...
function startWebsite() {
	...
	// DODAJEMY TO: Próba włączenia muzyki od razu
	if (!isPlaying) {
		toggleMusic();
	}
...
let matrixChars = 'WSZYSTKIEGONAJLEPSZEGO'.split('');
				...
				switch (action) {
					...
					case 'gift':
						...
						if (currentSettings.enableBook === true) {
							if (canvasEl && giftImage && matrixCanvas) {
							...
							if (
								...
								if (currentSettings.enableHeart === true) {
									setTimeout(() => startHeartEffect(), 2000);
								}
							} else {
					...
					default:
						S.Shape.switchShape(
							S.ShapeBuilder.letter(current[0] === cmd ? 'Co?' : current),
						);
				}
			},
...
function createPlaceholderImage(text) {
	return `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="300" height="400" viewBox="0 0 300 400"><rect width="300" height="400" fill="#f0f0f0"/><text x="150" y="200" text-anchor="middle" font-family="Arial" font-size="16" fill="#999">${text}</text></svg>`)}`;
}
...
function handleMouseMove(e) {
	...
	if (Math.abs(deltaX) > Math.abs(e.clientY - startY)) handleSwipeMove(deltaX);
}
...
function handleSwipeEnd(deltaX, deltaY, deltaTime) {
	...
	if (
		...
		(velocity > 0.5 && Math.abs(deltaX) > 30)
	) {
...
if (birthdayAudio) birthdayAudio.volume = 0.1;
...
function toggleMusic() {
	...
	if (isPlaying) {
		...
		if (musicControl) {
			...
			musicControl.title = 'Odtwórz muzykę';
		}
		isPlaying = false;
	} else {
		birthdayAudio
			...
			.then(() => {
				if (musicControl) {
					...
					musicControl.title = 'Zatrzymaj muzykę';
				}
				isPlaying = true;
			})
			.catch(() => {});
...
function cleanup() {
	if (typewriterTimeout) clearTimeout(typewriterTimeout);
	if (zIndexUpdateTimeout) clearTimeout(zIndexUpdateTimeout);
...
function calculatePageZIndexes() {
	if (!book) return;
	const pagesList = book.querySelectorAll('.page');
	const totalPages = pagesList.length;
	if (totalPages === 0) return;

	pagesList.forEach((page, physicalIndex) => {
		page.style.setProperty(
			'--page-z-index',
			(totalPages - physicalIndex).toString(),
		);
		page.style.setProperty(
			'--page-flipped-z-index',
			(physicalIndex + 1).toString(),
		);
	});
}
...
if (book) {
	...
	book.addEventListener('touchstart', forcePlayMusic, {
		passive: true,
		once: true,
	});
